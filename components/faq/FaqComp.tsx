...
const FaqComp: React.FC<Props> = ({ title, description }) => {
    ...
    return (
        <div className='my-6 flex flex-col items-center w-full relative'>
            <div className='font-header cursor-pointer flex shadow-card-upload-black md:w-[605px] w-[calc(100vw-50px)] text-lg font-semibold items-center justify-between bg-gradient-to-t from-[#3072ffaa] to-[#ca2ccf55] py-2 text-[white] px-8 rounded-lg z-30' onClick={() => setOpen(!open)}>
                {title}
